...
После каждого промпта Codex должен сделать небольшой PR/commit. Нельзя просить Codex "сделай всё сразу".
...
Я не могу мониторить репозиторий в фоне, поэтому ревью запускается только когда вы присылаете ссылку, diff или конкретный вопрос.
